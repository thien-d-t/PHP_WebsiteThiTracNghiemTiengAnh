...
if ($query) {
    $stm = $pdo->prepare("SELECT * FROM tbl_nguoidung WHERE email LIKE :query ORDER BY id_nguoidung");
    ...
    if ($stm->rowCount() > 0) {
        ...
        echo '<td style="text-align: center">Mã người dùng</td>';
        echo '<td>Họ tên</td>';
        echo '<td>Ngày sinh</td>';
        echo '<td style="text-align: center;">Email</td>';
        echo '<td style="text-align: center;">Tài khoản</td>';
        echo '<td>Mật khẩu</td>';
        ...
        foreach ($data as $item) {
            ...
            echo '<td style="text-align: center;">' . htmlspecialchars($item->id_nguoidung) . '</td>';
            echo '<td>' . htmlspecialchars($item->hoten) . '</td>';
            echo '<td style="text-align: center;">' . htmlspecialchars($item->ngaysinh) . '</td>';
            echo '<td style="text-align: center;">' . htmlspecialchars($item->email) . '</td>';
            echo '<td style="text-align: center;">' . htmlspecialchars($item->taikhoan) . '</td>';
            echo '<td style="text-align: center;">' . htmlspecialchars($item->matkhau) . '</td>';
            echo '<td style="text-align: center;">
                    <a href="edit_dethi.php?made=' . htmlspecialchars($item->id_nguoidung) . '">
                    ...
                    <a onclick="return confirm(\'Bạn có muốn xóa tài khoản này không???\');" href="delete_dethi.php?made=' . htmlspecialchars($item->id_nguoidung) . '">
        ...
        echo '<p class="attention" style="text-align: center;">Không tìm thấy tài khoản nào</p>';
    }
} else {
